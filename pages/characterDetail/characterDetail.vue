  ...
  <view class="character-detial-container">
    ...
    <uni-card class="basic-info-card" :isFull="true">
      ...
      <view class="level-content"
        >等级 {{ characterData.Level }} ({{ characterData.EXPPercent }}%)
      </view>
      <view class="global-rank sub-content"
        >全球排名
        {{
      ...
      <view class="server-rank sub-content"
        >服务器排名
        {{
    ...
    <uni-card
      v-if="characterData.GraphData && characterData.GraphData.length > 0"
      class="exp-info-card"
      title="日经验"
      :isFull="true"
    >
      <view class="exp-chart-container">
        <view class="detail last"
          >根据最新一天肝度，升级还需
          <text class="days">{{ needDays }}</text> 天</view
        >
    ...
    <view style="height: 8px" />
    <uni-card
      v-if="characterData.LegionLevel && characterData.LegionLevel >= 500"
      class="legion-info-card"
      :title="`${legionRankMap[legionLevel[0]]}联盟${legionLevel[1]}`"
      :thumbnail="iconPathMap[`${legionLevel[0]}_${legionLevel[1]}`]"
      :sub-title="characterData.LegionLevel + ''"
      :isFull="true"
    >
...
<script setup>
  ...
  import { legionRankMap, iconPathMap, jobMap } from '/constant/index.js';
  ...
  const needDays = computed(() => {
    if (characterData.value.GraphData) {
      ...
      const expToNextLevel =
        characterData.value.GraphData[graphDataLength - 1].EXPToNextLevel;
      const day1 = characterData.value.GraphData[graphDataLength - 2]
        .EXPDifference
        ? Math.ceil(
            expToNextLevel /
  ...
  const handleFavClick = () => {
    ...
    if (isFollowed.value) {
      userInfoStore.unFollowCharacter(
        baseInfo.value.name,
        baseInfo.value.region
      );
    } else {
